...
#   2. .tamago/tamago.conf  (legacy KEY=VALUE, pre-v2 backward compat)
#   3. ~/.tamago/machine.env  (global agent install, no project-level config)
...
# ─── Config discovery (3-step, first match wins) ─────────────────────────
...
# 2. Legacy fallback: grep-parse .tamago/tamago.conf (KEY=VALUE, pre-v2 format)
...
# 3. Global fallback: ~/.tamago/machine.env (for globally-installed agents)
if [ -z "$PROFILE_REPO" ]; then
