...
<profile-dir>/
  ...
  settings/
    claude/settings.json                 ← profile overrides only (no "agent" key)
    opencode/opencode.json               ← profile overrides only

<project-dir>/.tamago/
  tamago.conf                            ← generated by hatch; points to profile
```

The `"agent"` pointer is written into `settings.json` by `tamago install` (from
`tamago.conf`) — hatch no longer writes it into the profile settings file.
